...
public class Day13 extends AbstractDay {
    ...
    public static void main(String[] args) {
        ...
        long ans1 = sections.stream().map(CharTable::new).mapToLong(b -> getScore(b, 0, 0)).sum();
        long ans2 = sections.stream().map(CharTable::new).mapToLong(b -> getScore2(b)).sum();
    ...
    private static int getScore2(CharTable table) {
        ...
        for (var p : table.cells().toList()) {
            var copy = new CharTable(table);
            copy.set(p, table.get(p) == '.' ? '#' : '.');
    ...
    private static int findRowReflection(CharTable table, int skipRow) {
        return IntStream.range(1, table.height())
                .filter(i -> i != skipRow)
                .filter(i -> IntStream.range(0, min(i, table.height() - i))
                        .allMatch(j -> sameRow(table, i - j - 1, i + j)))
                .findFirst().orElse(0);
    }

    private static boolean sameRow(CharTable table, int i, int j) {
        return IntStream.range(0, table.width()).allMatch(x -> table.get(x, i) == table.get(x, j));
    }
